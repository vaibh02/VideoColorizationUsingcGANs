...
# Colorize YouTube video if URL provided
if youtube_url:
    if st.button("Colorize YouTube Video"):
        st.write("Colorizing YouTube video...")
        with st.spinner('Colorizing YouTube video...'):
            file_name = Path(youtube_url).name
            colorized_video_path = video_colorizer.colorize_from_url(youtube_url, render_factor=21, file_name=file_name)
            st.video(str(colorized_video_path))
            with open(colorized_video_path, 'rb') as f:
                st.download_button('Download Colorized Video', data=f, file_name=colorized_video_path.name)